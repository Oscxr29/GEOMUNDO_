  ...
  <main class="mx-auto max-w-6xl">
    <section class="geo-card rounded-[2rem] p-8 md:p-10">
      <div class="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
        <div>
          <span class="badge border-0 bg-(--coral)/30 px-3 py-2">Retroalimentación</span>
        ...
        <div class="rounded-[1.75rem] bg-white/80 p-6 shadow-sm">
          ...
          <div class="mt-4 space-y-4 text-slate-700">
            <div>
              ...
              <p class="mt-1 text-lg font-bold text-(--ink)">{{ progreso.temaSeleccionado || 'No seleccionado' }}</p>
            </div>
            <div>
              <p class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Actividad</p>
              <p class="mt-1 text-lg font-bold text-(--ink)">{{ progreso.actividadSeleccionada || 'No seleccionada' }}</p>
            </div>
            <div class="rounded-2xl bg-(--sky)/12 p-4 text-sm">
              Si algo no salió bien, regresa a la actividad y vuelve a intentarlo antes de guardar el resultado final.
            </div>
          </div>

          <div class="mt-6 flex flex-wrap gap-3">
            <button class="btn border-0 bg-(--mint) text-(--ink)" @click="router.push('/calificacion')">Ir a calificación</button>
